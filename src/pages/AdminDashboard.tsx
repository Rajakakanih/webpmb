...
export default function AdminDashboard() {
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, `Data_PMB_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  const printCard = (student: AdminData) => {
    ...
    doc.text("KARTU PENDAFTARAN PMB", 105, 20, { align: "center" });
    doc.setFontSize(14);
    ...
    doc.save(`Kartu_PMB_${student['No Pendaftaran']}.pdf`);
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            ...
            <p className={cn("mt-1", isDarkMode ? "text-slate-400" : "text-slate-500")}>Kelola data pendaftaran PMB {settings?.namaSekolah || 'Sekolah'}</p>
          </div>
                ...
                <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                  <thead className={isDarkMode ? "bg-slate-700 text-slate-200" : "bg-blue-50 text-blue-800"}>
                    <tr>
                      ...
                      <th scope="col" className="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider">Tanggal Daftar</th>
                    ...
                    {isLoading ? (
                      ...
                      currentData.map((item, idx) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            {formatDate(item['Tanggal Daftar']) || item['Tanggal Terakhir Pendaftaran'] || '-'}
                          </td>
