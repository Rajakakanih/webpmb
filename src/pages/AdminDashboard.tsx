...
import { Search, Filter, Download, Printer, CheckCircle, XCircle, Clock, FileText, Moon, Sun, Loader2, LogOut, Eye, X, Settings, LayoutDashboard, RefreshCw, Trash2 } from 'lucide-react';
import Swal from 'sweetalert2';
import * as XLSX from 'xlsx';
import jsPDF from 'jspdf';
import 'jspdf-autotable';
// Pastikan fungsi deleteRegistration sudah di-export dari file api.ts anda
import { getRegistrations, updateStatus, AdminData, updateSettings, deleteRegistration } from '../services/api';
...
export default function AdminDashboard() {
  ...
  // Fungsi Baru: Menghapus Data Pendaftar
  const handleDeleteStudent = async (noPendaftaran: string, namaLengkap: string) => {
    Swal.fire({
      title: 'Hapus Data?',
      text: `Apakah Anda yakin ingin menghapus data pendaftaran atas nama "${namaLengkap || 'pendaftar ini'}"? Tindakan ini tidak dapat dibatalkan.`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#d33',
      cancelButtonColor: '#3085d6',
      ...
      if (result.isConfirmed) {
        try {
          ...
          // Memanggil fungsi API deleteRegistration
          await deleteRegistration(noPendaftaran);

          // Update State local setelah berhasil hapus dari server
                        ...
                        >
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div className="flex items-center justify-end gap-2">
                              ...
                              <button onClick={() => handleDeleteStudent(item['No Pendaftaran'], getFieldValue(item, 'Nama Lengkap'))} className="text-red-600 hover:text-white hover:bg-red-600 bg-red-50 dark:bg-red-900/20 dark:hover:bg-red-700 px-2 py-1 rounded transition-colors" title="Hapus Data">
                                <Trash2 size={18} />
                              </button>
                            </div>
                          </td>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input
                        ...
                        className={cn("block w-full py-2 px-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm", isDarkMode ? "bg-slate-900 border-slate-700 text-white" : "bg-white border-slate-300 text-slate-900")}
                      />
                    </div>
                  </div>
                )}
