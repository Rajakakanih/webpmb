/**
 * Google Apps Script Backend for PMB MTs
 * Deploy as a Web App:
...
function setup() {
  ...
  if (!adminSheet) {
    ...
    adminSheet.appendRow(["admin", "T4kafful100%"]); // Default credentials
...
function handleRegistration(data) {
  ...
  const noPendaftaran = `PMB-${year}-${String(nextId).padStart(3, '0')}`;
